...
public class AdmitStudentController {
    ...
    public Student admitStudent(
        ...
        student.setCnic(request.getCnic());
    ...
    @PostMapping
    public Student admitStudentWithoutUser(
            @RequestBody StudentRequest request,
            @RequestParam Long hostelId
    ) {
        Hostel hostel = getHostel(hostelId);
        ...
        Student student = new Student();
        student.setUser(null); // important
    ...
    public List<Student> getAllStudentsWithOutUser(@RequestParam Long hostelId) {
        return studentRepository.findAllByHostel_Id(hostelId)
                .stream()
